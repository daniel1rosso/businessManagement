    ...
    public function login() {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if(empty($user) || empty($password)) {
                $data['user'] = (empty($user)) ? null :  $username;
                $data['password'] = (empty($password)) ? null : $password;

                $dato['msg'] = "Usuario o contraseña se encuentra vacio";
                $dato['valid'] = false;
            } else {
                $result = $this->app_model->compare_user_password_public_admin($username, $password);
